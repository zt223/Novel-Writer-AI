...
export async function generateCharacterProfilesAndMap(context: string): Promise<string> {
    const prompt = `你是一位资深的网络小说角色设计师。根据小说的核心设定，为主角、关键女角色（可以是女主，也可以是重要女配）、以及2-3个核心配角（可以是盟友、导师或反派）创建一份整合式的人物档案，并生成一份清晰的人物关系图谱。
...
**第一部分：核心人物档案 (使用中文Markdown)**

#### 1. 主角
- **姓名:**
- **外貌与风度:** (简述，突出记忆点)
- **性格:** (基于核心设定中的原型，并增加1-2个具体特质或矛盾点)
- **背景故事:** (简述其出身和导致他走上故事开端的关键经历)
- **主要目标/动机:** (他在故事中最想得到什么？)

#### 2. 关键女角色
- **姓名与身份:** (例如：没落宗门的大小姐 / 神秘的商会会长)
- **外貌与气质:**
- **性格特点:**
- **与主角的关系:** (例如：青梅竹马、宿敌、师徒、一见钟情但有误会、利益交换的盟友)
- **在故事中的作用:** (是主角的引导者？还是麻烦的制造者？)

#### 3. 核心配角 #1
- **定位:** (例如：忠诚的挚友 / 神秘的导师 / 早期的竞争对手)
- **姓名与身份:**
- **核心性格:**
- **与主角的关系及功能:** (例如：作为主角的后盾提供资源 / 向主角传授关键技能 / 通过竞争迫使主角成长)

#### 4. 核心配角 #2
- **定位:** (例如：搞笑担当 / 主要反派 / 亦敌亦友的角色)
- **姓名与身份:**
- **核心性格:**
- **与主角的关系及功能:**

**第二部分：人物关系图谱 (Visual Relationship Map)**

**指示:** 基于以上生成的人物，用清晰的列表形式构建一个视觉化的关系图谱。

### 人物关系图谱

- **[主角姓名]**
    - ➡ **[关键女角色姓名]**: **[关系类型]** (例如: 宿命纠缠) - *简要描述关系动态*
    - ➡ **[配角#1姓名]**: **[关系类型]** (例如: 绝对信任) - *简要描述关系动态*
    - ➡ **[配角#2姓名]**: **[关系类型]** (例如: 亦敌亦友) - *简要描述关系动态*

- **[关键女角色姓名]**
    - ➡ **[配角#1姓名]**: **[关系类型]** (例如: 家族世仇) - *简要描述关系动态*
`;
    return callGemini(prompt);
}
